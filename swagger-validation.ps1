...
# Run validation
$validationPassed = Test-SwaggerSchema
...
elseif ($OutputFormat -eq "Xml") {
    ...
    $root = $result.CreateElement("SwaggerValidation")
    $result.AppendChild($root) | Out-Null

    $successNode = $result.CreateElement("Success")
    $successNode.InnerText = $validationPassed.ToString()
    $root.AppendChild($successNode) | Out-Null

    $errorsNode = $result.CreateElement("Errors")
    foreach ($error in $validationErrors) {
        $errorNode = $result.CreateElement("Error")
        $errorNode.InnerText = $error
        $errorsNode.AppendChild($errorNode) | Out-Null
    }
    $root.AppendChild($errorsNode) | Out-Null

    $warningsNode = $result.CreateElement("Warnings")
    foreach ($warning in $validationWarnings) {
        $warningNode = $result.CreateElement("Warning")
        $warningNode.InnerText = $warning
        $warningsNode.AppendChild($warningNode) | Out-Null
    }
    $root.AppendChild($warningsNode) | Out-Null

    $infoNode = $result.CreateElement("Info")
    foreach ($info in $validationInfo) {
        $infoItemNode = $result.CreateElement("InfoItem")
        $infoItemNode.InnerText = $info
        $infoNode.AppendChild($infoItemNode) | Out-Null
    }
    $root.AppendChild($infoNode) | Out-Null
