...
- **Phase 4 (US2: Program Matching)**: 10 tasks (T031-T040)
- **Phase 5 (US3: State-Specific Rules)**: 8 tasks (T041-T048)
- **Phase 6 (US4: Plain-Language Explanations)**: 9 tasks (T049-T057)
- **Phase 7 (US5: FPL Integration)**: 8 tasks (T058-T065)
- **Phase 8 (US6: Pathway Identification)**: [P] 3 tasks (T066-T068) ✓ Parallelizable with Phase 3
...
- [ ] T026 Create src/MAA.Tests/Unit/Rules/RuleEngineTests.cs with 8+ test cases:
...
- [ ] T029 Create src/MAA.Tests/Integration/RulesApiIntegrationTests.cs with 8+ test cases:
  ...
  - Determinism: same input evaluated twice → identical output
...
- [ ] T037 Create src/MAA.Tests/Contract/RulesApiContractTests.cs (extension) validating:
...
- [ ] T038 Create src/MAA.Application/Eligibility/Services/StateRuleLoader.cs: loadRulesForState(stateCode: string) → List<EligibilityRule> (with cache)
- [ ] T039 [P] Create src/MAA.Infrastructure/Data/Rules/StateRuleRepository.cs: GetAllProgramsForState(stateCode), GetProgramsWithRules(stateCode, effectiveDate)
- [ ] T040 [P] Extend RuleCacheService with state-scoped cache: rules cached by state_code key, invalidated per-state

### Unit Tests for US3

- [ ] T041 Create src/MAA.Tests/Unit/Eligibility/StateRuleLoaderTests.cs with 6+ test cases:
...
- [ ] T042 Create src/MAA.Tests/Integration/RulesApiIntegrationTests.cs (extension) with 6+ test cases:
...
- [ ] T043 [P] Create state-specific test data: src/MAA.Tests/Data/pilot-states-test-cases.json with IL, CA, NY, TX, FL scenarios
- [ ] T044 [P] Extend RulesApiContractTests.cs validating state_code parameter validation in requests
...
- [ ] T045 Create src/MAA.Domain/Rules/ExplanationGenerator.cs pure function with multiple methods:
...
- [ ] T046 [P] Create src/MAA.Domain/Rules/JargonDefinition.cs static dictionary mapping: MAGI → "Modified Adjusted Gross Income", FPL → "Federal Poverty Level", SSI → "Social Security Income"
- [ ] T047 [P] Create src/MAA.Application/Eligibility/Validators/ReadabilityValidator.cs: ScoreReadability(text: string) → FleschKincaidScore, IsBelow8thGrade() method

### Unit Tests for US4

- [ ] T048 Create src/MAA.Tests/Unit/Rules/ExplanationGeneratorTests.cs with 10+ test cases:
...
- [ ] T049 [P] Create src/MAA.Tests/Unit/Rules/JargonDefinitionTests.cs with 4+ test cases:
...
- [ ] T050 [P] Create src/MAA.Tests/Unit/Eligibility/ReadabilityValidatorTests.cs with 5+ test cases:
...
- [ ] T051 Create src/MAA.Tests/Integration/RulesApiIntegrationTests.cs (extension) with 6+ test cases:
...
- [ ] T052 [P] Extend RulesApiContractTests.cs validating explanation field:
...
- [ ] T053 Create src/MAA.Application/Eligibility/Services/FPLThresholdCalculator.cs: CalculateThreshold(fplAmount: decimal, percentage: int) → decimal, GetFPLForYear(year: int, householdSize: int, stateCode?: string) → FPL
- [ ] T054 [P] Extend FPLRepository.cs with: GetCurrentYearFPL(householdSize, stateCode), GetFPLRange(year, householdSizes: List<int>), HasFPLForYear(year)
- [ ] T055 [P] Create src/MAA.Infrastructure/Caching/FPLCacheService.cs: In-memory cache for FPL tables with 1-year TTL, refresh on year boundary

### Unit Tests for US5

- [ ] T056 Create src/MAA.Tests/Unit/Eligibility/FPLThresholdCalculatorTests.cs with 10+ test cases:
...
- [ ] T057 Create src/MAA.Tests/Integration/RulesApiIntegrationTests.cs (extension) with 7+ test cases:
...
- [ ] T058 [P] Create src/MAA.Tests/Data/fpl-2026-test-data.json with 8 household sizes (1-8+) and baseline FPL amounts
- [ ] T059 [P] Extend RulesApiContractTests.cs validating FPL endpoint: GET /api/fpl?year=2026&state_code=IL returns correctly structured FPL data

---

## Phase 8: US6 - Eligibility Pathway Identification (P2)

**Story Goal**: System determines which pathway(s) apply (MAGI, non-MAGI, SSI, aged, disability, pregnancy) based on user characteristics
...
- [ ] T060 [P] Create src/MAA.Domain/Rules/PathwayIdentifier.cs pure function: DetermineApplicablePathways(input: UserEligibilityInput) → List<EligibilityPathway> enum
- [ ] T061 [P] Create src/MAA.Domain/Rules/PathwayRouter.cs: RouteToPrograms(pathways: List<EligibilityPathway>, state: string) → List<MedicaidProgram> (filter programs by pathway)
- [ ] T062 [P] Create src/MAA.Application/Eligibility/Services/PathwayEvaluationService.cs orchestrator integrating pathway identification with rule evaluation

### Unit Tests for Phase 8

- [ ] T063 Create src/MAA.Tests/Unit/Rules/PathwayIdentifierTests.cs with 8+ test cases:
...
- [ ] T064 Create src/MAA.Tests/Integration/RulesApiIntegrationTests.cs (extension) with 5+ test cases:
...
- [ ] T065 [P] Extend RulesApiContractTests.cs validating pathway field in matched_programs responses
...
- T029, T042, T051, T057: Integration tests verify rule version recorded in evaluations

### Unit Tests for Versioning

- [ ] T066 Create src/MAA.Tests/Unit/Rules/RuleVersioningTests.cs with 8+ test cases:
...
- [ ] T067 Create src/MAA.Tests/Integration/RulesApiIntegrationTests.cs (extension) with 5+ test cases:
...
- [ ] T068 [P] Extend RulesApiContractTests.cs validating:
...
- **Phase 4**: T031-T033 (logic) parallel; T034-T037 (tests) parallel
- **Phase 5-7**: Similar pattern; logic tasks parallel, tests parallel
- **Phase 8**: T060-T062 logic tasks fully parallel (no interdependencies)
...
Phase 3 (US1: Basic Evaluation) ←→ Phase 5 (US5: FPL) - both needed for accurate thresholds
    ↓
Phase 4 (US2: Multi-Program) - depends on US1 core logic
    ↓
Phase 6 (US4: Explanations) - depends on US1-US2 results
Phase 8 (US6: Pathways) - can start after Phase 3, improves routing
...
- ✅ All 68 tasks completed and committed to 002-rules-engine branch
- ✅ Core evaluation logic (RuleEngine, FPLCalculator, ProgramMatcher) pure functions with 0 dependencies
- ✅ All 7 user stories have independent test criteria verified
- ✅ Unit test coverage ≥80% for domain logic (rules, calculations, matching)
...
- ✅ Performance targets met: eligibility evaluation ≤2 seconds (p95), FPL lookup <10ms
- ✅ Constitutional compliance: Code Quality ✅, Testing ✅, UX ✅, Performance ✅
- ✅ Documentation complete: Explanations readable at 8th-grade level, no unexplained jargon
- ✅ Ready for Phase 3: Next phase generates rule content (pilot state rules, FPL data seeding)

---

## Next Steps (After Task Completion)

1. **Phase 0 Research Execution** (Parallel with task implementation):
   - Research team gathers Medicaid rules for 5 pilot states (IL, CA, NY, TX, FL)
   - Obtain 2026 FPL tables from HHS
   - Finalize JSONLogic.Net library integration decision
   - Design explanation templates with jargon dictionary
