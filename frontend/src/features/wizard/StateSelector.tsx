  ...
  useEffect(() => {
    const loadStates = async () => {
      try {
        ...
        setStatesError(
          "Failed to load available states. Please refresh the page.",
        );
      } finally {
  ...
  const handleZipLookup = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        setZipError(
          "ZIP code not found or state not available in pilot program",
        );
      }
    } catch (err) {
      console.error("ZIP lookup failed:", err);
      setZipError(
        "Failed to lookup ZIP code. Please select your state manually.",
      );
    } finally {
    ...
    <fieldset className="space-y-6" disabled={disabled}>
      ...
      <div className="space-y-3">
        <Label htmlFor="state-select" className="font-medium">
          <span className="font-semibold">Select Your State</span>{" "}
        ...
        >
          ...
          >
            <SelectValue
              placeholder={
                isLoadingStates ? "Loading states..." : "Choose a state"
              }
            />
          </SelectTrigger>
          <SelectContent>
