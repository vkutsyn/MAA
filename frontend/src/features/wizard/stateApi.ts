 ...
 */
export async function lookupStateByZip(zip: string): Promise<StateLookupResponse | null> {
  try {
    const response = await apiClient.get<StateLookupResponse>(`/states/lookup`, {
      params: { zip },
    })
    return response.data
  } catch (error: any) {
